...
const loginUser = async (req: Request, res: Response) => {
  try {
    ...
    const loginUser: IUser = (await User.findOne({ $or: [{ email: identifier }, { username: identifier }] })) as IUser;

    if (!loginUser) {
      return res.status(403).json({ success: false, message: `user ${identifier} does not exist.}` });
    }
    ...
    const accessToken = jwt.sign({ userId: loginUser._id!.toString() }, env.ACCESS_TOKEN as string, { expiresIn: "1800s" });
